...
def search(self):
    ...
    if query is not None:
        xbmc.executebuiltin('Container.Update(plugin://plugin.video.sling2/?mode=search&query=%s)' % query)
    else:
                ...
                for ribbon in ribbons:
                    if "SHOWS" in ribbon['title']:
                        for sling_show in ribbon['tiles']:
                            ...
                            if show.Name != '':
                                context_items = [
                                    ('Add to Favorite Shows', 'RunPlugin(plugin://plugin.video.sling2/?mode=show&guid=%s&'
                                                              'action=favorite)' % show.GUID),
                                    ('Update Show', 'RunPlugin(plugin://plugin.video.sling2/?mode=show&guid=%s&'
