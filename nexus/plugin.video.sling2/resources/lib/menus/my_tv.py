...
def myTVChannel(self, tile):
    ...
    if asset['GUID'] != '':
        ...
        if channel.infoLabels()['duration'] > 0 and 'OFF-AIR' not in channel.infoLabels()['plot']:
            # ======================== Live Channel ========================
            context_items = [('Hide Channel', 'RunPlugin(plugin://plugin.video.sling2/?'
            ...
            # ======================== On Demand Channel ========================
            context_items = [
                ('Update On Demand Content', 'RunPlugin(plugin://plugin.video.sling2/?mode=demand&guid=%s&'
    ...
    if asset['GUID'] != '' and asset['URL'] != '':
        ...
        if response is not None and response.status_code == 200:
            ...
            if asset['Show_GUID'] != '' and (asset['Name'] != asset['Show_Name']) and \
                ...
                context_items = [
                    ('View Show', 'Container.Update(plugin://plugin.video.sling2/?mode=show&guid=%s&name=%s)' %
...
def myTVShow(self, tile):
    ...
    if asset['GUID'] != '':
        ...
        if show.Name != "":
            context_items = [
                ('Add to Favorite Shows', 'RunPlugin(plugin://plugin.video.sling2/?mode=show&guid=%s&action=favorite)' % show.GUID),
                ('Update Show', 'RunPlugin(plugin://plugin.video.sling2/?mode=show&guid=%s&action=update)' % show.GUID)
            ]
...
def myTVRecording(self, tile, session):
    ...
    if asset['URL'] != '':
        ...
        if response.status_code == 200:
            ...
            context_items = []
            if asset['Show_GUID'] != '':
                context_items.append(('View Show', 'Container.Update(plugin://plugin.video.sling2/?mode=show&guid=%s&'
