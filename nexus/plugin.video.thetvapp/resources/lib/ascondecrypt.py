...
def tvapp(ciphertext, associateddata,key):
	encoded_buffer = bytes.fromhex(ciphertext)
	associateddata =  b'"'+associateddata.encode('utf-8')+b'"'
	kk=(ascon_hash(key.encode('utf-8'), variant="Ascon-Xof", hashlength=32))
	url = ascon_decrypt(kk[0:16], encoded_buffer[-16:],associateddata, encoded_buffer[0:-16], variant="Ascon-128")
	return url
