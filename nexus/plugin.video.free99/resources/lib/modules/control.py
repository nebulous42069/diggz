...
def artPath():
    ...
    #elif condVisibility('System.HasAddon(script.Free99.artwork)'):
#        return os.path.join(xbmcaddon.Addon('script.Free99.artwork').getAddonInfo('path'), 'resources', 'media', theme)
    return os.path.join(xbmcaddon.Addon('plugin.video.free99').getAddonInfo('path'), 'resources', 'media', theme)

def appearance():
    appearance = setting('appearance.1').lower()
    #appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.Free99.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://plugin.video.free99)')
