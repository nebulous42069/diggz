...
class sources:
    ...
    def getConstants(self):
        ...
        self.hostblockDict = ['clipwatching.com', 'dailyuploads.net', 'estream.to', 'fruitadblock.net',
            ...
            'vidtodoo.com', 'vshare.eu', 'vshare.io', 'youtube.com', 'youtu.be', 'youtube-nocookie.com'
            'database.gdriveplayer.us', 'databasegdriveplayer.co', 'series.databasegdriveplayer.co',
            'aparat.cam', 'wolfstream.tv', 'rabbitstream.net', 'dokicloud.one'
        ]
        self.hostblockDictALT = ['dailyuploads.net', 'estream.to', 'fruitadblock.net',
            'hqq.to', 'hydrax.net', 'hydrax.xyz', 'netu.tv', 'openload.co', 'speedvid.net',
            'streamango.com', 'streamcherry.com', 'subscene.com', 'verystream.com', 'vidlox.me',
            'vidtodoo.com', 'vshare.io', 'youtube.com', 'youtu.be', 'youtube-nocookie.com'
        ]
        if control.setting('sourcefilter.hostblocks') == 'true':
            self.hostblockDict = self.hostblockDictALT
        else:
            self.hostblockDict = self.hostblockDict
    ...
    def sourcesFilter(self, _content, sort=False):
        ...
        try:
            if remove_dupes == 'true' and len(self.sources) > 1:
                #self.sources = sorted(self.sources, key=lambda k: k['provider'])  # removed thinking it adds too much wait time.
        ...
        try:
            ...
            for i in sourceDict:
                ...
                try:
                    ...
                    domains_label = '[CR]'
                scraper_label = '[B]%s :[/B]  %s %s' % (provider, base_link, domains_label)
                list.append(scraper_label)
            #log_utils.log('getProviderDomains list: \n' + repr(list))
