...
class movies:
    def __init__(self):
        ...
        self.imdb_watchlist_link = self.imdb_link + '/user/ur%s/' % self.imdb_user + 'watchlist?sort=date_added%2Cdesc'
        self.imdb_watchlist2_link = self.imdb_link + '/user/ur%s/' % self.imdb_user + 'watchlist?sort=alpha%2Casc'
...
# UserLists Frakenstein
    ...
    def search_imdb_movie(self, url):
        try:
            ...
            result = client_utils.parseDOM(result, 'table', attrs={'class': 'findList'})[0]
            items = client_utils.parseDOM(result, 'td', attrs={'class': 'result_text'})
            for item in items:
                try:
                    title = client_utils.parseDOM(item, 'a')[0]
        ...
        try:
            ...
            for item in items:
                try:
                    ...
                    year += client_utils.parseDOM(item, 'span', attrs={'class': 'year_type'})
                    try:
    ...
    def final_info(self, i):
        try:
            ...
            if premiered:
                ...
                premiered = '0'
            if self.studio_artwork == 'true': ## Changed these lines of code a bit for the kodi 20 list item changes lol.
                ...
                studio = []#'0'
            genre = item['genres'] ## Changed these lines of code a bit for the kodi 20 list item changes lol.
            if genre:
            ...
            try:
                ...
                mpaa = '0'
            director = item['credits']['crew'] ## Changed these lines of code a bit for the kodi 20 list item changes lol.
            try:
                director = [x['name'] for x in director if str(x['job']) == 'Director']
                #director = ' / '.join(director).strip()
            except:
                director = []#'0'
            writer = item['credits']['crew'] ## Changed these lines of code a bit for the kodi 20 list item changes lol.
            try:
        ...
        try:
            ...
            if u in self.tmdb_link and ('/list/' in url or '/collection/' in url):
                ...
                if self.addon_caching == 'true':
                    ...
                    self.list = self.search_imdb_movie(url)
                if idx == True:
                    self.worker()
