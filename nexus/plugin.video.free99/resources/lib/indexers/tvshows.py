...
# User Lists BlairWitch
    ...
    def imdb_list(self, url):
        try:
            ...
            items = client_utils.parseDOM(result, 'div', attrs = {'class': 'lister-item .*?'})
            items += client_utils.parseDOM(result, 'div', attrs = {'class': 'list_item.*?'})
        except:
        ...
        try:
            ...
            next = client_utils.replaceHTMLCodes(six.ensure_str(next))
        except:
            next = ''
        for item in items:
            try:
    ...
    def imdb_person_list(self, url):
        ...
        for item in items:
            try:
                ...
                url = client_utils.replaceHTMLCodes(six.ensure_str(url))
                image = client_utils.parseDOM(item, 'img', ret='src')[0]
                image = re.sub('(?:_SX|_SY|_UX|_UY|_CR|_AL)(?:\d+|_).+?\.', '_SX500.', image)
                image = client_utils.replaceHTMLCodes(six.ensure_str(image))
    ...
    def final_info(self, i):
        try:
            ...
            if self.hq_artwork == 'true':
                poster2, fanart2, banner, clearlogo, clearart, landscape = self.get_fanart_tv_artwork(imdb)
            else:
                poster2 = fanart2 = banner = clearlogo = clearart = landscape = '0'
    ...
    def tvshowDirectory(self, items):
        ...
        watchedMenu = '[I]Watched in Trakt[/I]' if trakt.getTraktIndicatorsInfo() == True else '[I]Watched in Free99[/I]'
        unwatchedMenu = '[I]Unwatched in Trakt[/I]' if trakt.getTraktIndicatorsInfo() == True else '[I]Unwatched in Free99[/I]'
