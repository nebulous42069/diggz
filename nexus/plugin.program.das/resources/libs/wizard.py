...
try:
    import downloader,extract,skinSwitch 
except ImportError:
    from resources.libs import downloader,extract,skinSwitch
...
try:
    ...
    from urllib import quote, urlencode, quote_plus, addinfourl, unquote_plus
    import urllib2
...
def basecode(text, encode=True):
    ...
    if encode == True:
        ...
        msg = base64.decodestring(text.encode('utf-8'))
    return msg
...
def toggleAddon(id, value, over=None):
    ...
    if 'error' in response and over == None:
        ...
        DIALOG.ok(ADDONTITLE, "[COLOR %s]Error %s [COLOR %s]%s[/COLOR]Check to make sure the addon list is upto date and try again.[/COLOR]" % (COLOR2, v, COLOR1 , id))
        forceUpdate()
...
def createQR():
    ...
    if not working == True:
        if not DIALOG.yesno(ADDONTITLE, "[COLOR %s]It seems the your enter isnt working, Would you like to create it anyways?[/COLOR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, working), yeslabel="[B][COLOR red]Yes Create[/COLOR][/B]", nolabel="[B][COLOR springgreen]No Cancel[/COLOR][/B]"):
            return
    ...
    DIALOG.ok(ADDONTITLE, "[COLOR %s]The QRCode image has been created and is located in the addondata directory:[/COLOR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, image.replace(HOME, '')))
...
def cleanupBackup():
    ...
    if selected == -1:
        ...
        if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to clean up all items in your 'My_Builds' folder?[/COLOR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, MYBUILDS), yeslabel="[B][COLOR springgreen]Clean Up[/COLOR][/B]", nolabel="[B][COLOR red]No Cancel[/COLOR][/B]"):
        ...
        if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to remove [COLOR %s]%s[/COLOR] from 'My_Builds' folder?[/COLOR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, list[selected], COLOR1, path), yeslabel="[B][COLOR springgreen]Clean Up[/COLOR][/B]", nolabel="[B][COLOR red]No Cancel[/COLOR][/B]"):
...
def convertSpecial(url, over=False):
    ...
    DP.create(ADDONTITLE, "[COLOR %s]Changing Physical Paths To SpecialPlease Wait[/COLOR]" % COLOR2)
    for root, dirs, files in os.walk(url):
        for file in files:
            start += 1
            perc = int(percentage(start, total))
            if file.endswith(".xml") or file.endswith(".hash") or file.endswith("properies"):
                DP.update(perc, "[COLOR %s]Scanning: [COLOR %s]%s[/COLOR][COLOR %s]%s[/COLOR]Please Wait[/COLOR]" % (COLOR2, COLOR1, root.replace(HOME, ''), COLOR1, file))
                a = xbmcvfs.File(os.path.join(root, file)).read()
                ...
                f = xbmcvfs.File((os.path.join(root, file)), 'w')
                f.write(str(b))
                f.close()
                if DP.iscanceled():
                    DP.close()
...
def clearCrash():
    ...
    for file in glob.glob(os.path.join(LOG, '*crashlog*.*')):
        files.append(file)
    if len(files) > 0:
        if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Would you like to delete the Crash logs?[COLOR %s]%s[/COLOR] Files Found[/COLOR]' % (COLOR2, COLOR1, len(files)), yeslabel="[B][COLOR springgreen]Remove Logs[/COLOR][/B]", nolabel="[B][COLOR red]Keep Logs[/COLOR][/B]"):
            for f in files:
                xbmcvfs.delete(f)
...
def convertText():
    ...
    if not BUILDFILE == 'http://':
        ...
        DP.update(0,'[COLOR %s][B]Converting Text:[/B][/COLOR] [COLOR %s]Builds.txt[/COLOR]Please Wait' % (COLOR2, COLOR1))
        ...
        if len(match2) == 0:
            for name, version, url, gui, kodi, theme, icon, fanart in match:
                ...
                if not theme == 'http://':
                    ...
                    DP.update(0,'[COLOR %s][B]Converting Text:[/B][/COLOR] [COLOR %s]%s_theme.txt[/COLOR]Please Wait' % (COLOR2, COLOR1, name))
    ...
    if not APKFILE == 'http://':
        ...
        DP.update(0,'[COLOR %s][B]Converting Text:[/B][/COLOR] [COLOR %s]Apks.txt[/COLOR]Please Wait' % (COLOR2, COLOR1))
        ...
        if len(match2) == 0:
            for name, url, icon, fanart in match:
                x += 1
                DP.update(int(percentage(x, len(match))), "[COLOR %s][B]Converting Text:[/B][/COLOR] [COLOR %s]Apks.txt[/COLOR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, COLOR1, name))
                if not writing == '': writing += '\n'
                writing += 'name="%s"\n' % name
                writing += 'section="no"'
                writing += 'url="%s"\n' % url
            ...
            for name, url, icon, fanart, adult, description in match2:
                x += 1
                DP.update(int(percentage(x, len(match2))), "[COLOR %s][B]Converting Text:[/B][/COLOR] [COLOR %s]Apks.txt[/COLOR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, COLOR1, name))
                if not writing == '': writing += '\n'
                writing += 'name="%s"\n' % name
                writing += 'section="no"'
                writing += 'url="%s"\n' % url
    ...
    if not YOUTUBEFILE == 'http://':
        ...
        DP.update(0,'[COLOR %s][B]Converting Text:[/B][/COLOR] [COLOR %s]YouTube.txt[/COLOR]Please Wait' % (COLOR2, COLOR1))
        match = re.compile('name="(.+?)".+?rl="(.+?)".+?con="(.+?)".+?anart="(.+?)".+?escription="(.+?)"').findall(a)
        for name, url, icon, fanart, description in match:
            x += 1
            DP.update(int(percentage(x, len(match))), "[COLOR %s][B]Converting Text:[/B][/COLOR] [COLOR %s]YouTube.txt[/COLOR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1 ,COLOR1, name))
            if not writing == '': writing += '\n'
            writing += 'name="%s"\n' % name
            writing += 'section="no"'
            writing += 'url="%s"\n' % url
    ...
    if not ADVANCEDFILE == 'http://':
        ...
        DP.update(0,'[COLOR %s][B]Converting Text:[/B][/COLOR] [COLOR %s]AdvancedSettings.txt[/COLOR]Please Wait' % (COLOR2, COLOR1))
        match = re.compile('name="(.+?)".+?rl="(.+?)".+?con="(.+?)".+?anart="(.+?)".+?escription="(.+?)"').findall(a)
        for name, url, icon, fanart, description in match:
            x += 1
            DP.update(int(percentage(x, len(match))), "[COLOR %s][B]Converting Text:[/B][/COLOR] [COLOR %s]AdvancedSettings.txt[/COLOR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, COLOR1, name))
            if not writing == '': writing += '\n'
            writing += 'name="%s"\n' % name
            writing += 'section="no"'
            writing += 'url="%s"\n' % url
...
def asciiCheck(use=None, over=False):
    ...
    for base, dirs, files in os.walk(source):
        ...
        for file in files:
            ...
            DP.update(prog2,"[COLOR %s]Checking for non ASCII files[COLOR %s]%s[/COLOR]Please Wait[/COLOR]" % (COLOR2, COLOR1, file))
            try:
                file.encode('ascii')
...
def backUpOptions(type, name=""):
    ...
    try:
        ...
        DIALOG.ok(ADDONTITLE, "[COLOR %s]Error making Back Up directories:[/COLOR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, str(e)))
        return
    if type == "addon pack":
        if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Are you sure you wish to create an Addon Pack?[/COLOR]" % COLOR2, nolabel="[B][COLOR red]Cancel Backup[/COLOR][/B]", yeslabel="[B][COLOR springgreen]Create Pack[/COLOR][/B]"):
            ...
            log(selected)
            DP.create(ADDONTITLE,'[COLOR %s][B]Creating Zip File:[/B][/COLOR]Please Wait' % COLOR2)
            if len(selected) > 0:
                added = []
                for item in selected:
                    added.append(addonfolds[item])
                    DP.update(0, "[COLOR %s][B]Creating Zip File:[/B][/COLOR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, addonfolds[item]))
                    for base, dirs, files in os.walk(os.path.join(ADDONS,addonfolds[item])):
                        files[:] = [f for f in files if f not in exclude_files]
                        for file in files:
                            if file.endswith('.pyo'): continue
                            DP.update(0, "[COLOR %s][B]Creating Zip File:[/B][/COLOR][COLOR %s]%s[/COLOR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, addonfolds[item], COLOR1, file))
                    ...
                    if os.path.exists(dep):
                        ...
                        for depends in match:
                            ...
                            DP.update(0, "[COLOR %s][B]Creating Zip File:[/B][/COLOR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, depends))
                            for base, dirs, files in os.walk(os.path.join(ADDONS,depends)):
                                files[:] = [f for f in files if f not in exclude_files]
                                for file in files:
                                    if file.endswith('.pyo'): continue
                                    DP.update(0, "[COLOR %s][B]Creating Zip File:[/B][/COLOR][COLOR %s]%s[/COLOR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, depends, COLOR1, file))
                                    fn = os.path.join(base, file)
                                    zipf.write(fn, fn[len(ADDONS):], zipfile.ZIP_DEFLATED)
        ...
        if os.path.exists(GUISETTINGS):
            ...
            try:
                ...
                for fold in match:
                    fd = os.path.split(fold[:-1])[1]
                    if not fd in ['skin.confluence', 'skin.re-touch', 'skin.estuary', 'skin.estouchy']:
                        if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to add the following skin folder to the GuiFix Zip File?[/COLOR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, fd), yeslabel="[B][COLOR springgreen]Add Skin[/COLOR][/B]", nolabel="[B][COLOR red]Skip Skin[/COLOR][/B]"):
                            for base, dirs, files in os.walk(os.path.join(ADDOND,fold)):
                                files[:] = [f for f in files if f not in exclude_files]
                                for file in files:
                                    fn = os.path.join(base, file)
        ...
        if name == "":
            DIALOG.ok(ADDONTITLE, "[COLOR %s]GuiFix backup successful:[/COLOR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, guizipname))
    elif type == "theme":
        ...
        try:
            if not SKIN in ['skin.confluence', 'skin.estuary']:
                ...
                if len(match2) > 1:
                    if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Would you like to go through the Texture Files for?[/COLOR]" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, SKIN), yeslabel="[B][COLOR springgreen]Add Textures[/COLOR][/B]", nolabel="[B][COLOR red]Skip Textures[/COLOR][/B]"):
                        ...
                        match2 = glob.glob(os.path.join(skinfold,'*.xbt'))
                        for xbt in match2:
                            if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Would you like to add the Texture File [COLOR %s]%s[/COLOR]?from [COLOR %s]%s[/COLOR][/COLOR]" % (COLOR1, COLOR2, xbt.replace(skinfold, "")[1:], COLOR1, SKIN), yeslabel="[B][COLOR springgreen]Add Textures[/COLOR][/B]", nolabel="[B][COLOR red]Skip Textures[/COLOR][/B]"):
                                fn  = xbt
                                fn2 = fn.replace(HOME, "")
                                zipf.write(fn, fn2, zipfile.ZIP_DEFLATED)
                else:
                    for xbt in match2:
                        if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Would you like to add the Texture File [COLOR %s]%s[/COLOR]?from [COLOR %s]%s[/COLOR][/COLOR]" % (COLOR2, COLOR1, xbt.replace(skinfold, "")[1:], COLOR1, SKIN), yeslabel="[B][COLOR springgreen]Add Textures[/COLOR][/B]", nolabel="[B][COLOR red]Skip Textures[/COLOR][/B]"):
                            fn  = xbt
                            fn2 = fn.replace(HOME, "")
                            zipf.write(fn, fn2, zipfile.ZIP_DEFLATED)
                ad_skin = os.path.join(ADDOND, SKIN, 'settings.xml')
                if os.path.exists(ad_skin):
                    if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Would you like to go add the [COLOR %s]settings.xml[/COLOR] in [COLOR %s]/addon_data/[/COLOR] for?[COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, COLOR1, COLOR1, SKIN), yeslabel="[B][COLOR springgreen]Add Settings[/COLOR][/B]", nolabel="[B][COLOR red]Skip Settings[/COLOR][/B]"):
            ...
            DIALOG.ok(ADDONTITLE, "[COLOR %s]%s[/COLOR][COLOR %s] theme zip failed:[/COLOR][COLOR %s]%s[/COLOR]" % (COLOR1, themename, COLOR2, COLOR1, str(e)))
            if not tempzipname == '':
        ...
        if not tempzipname == '':
            ...
            if success == 0:
                xbmcvfs.copy(tempzipname, zipname)
                xbmcvfs.delete(tempzipname)
        DIALOG.ok(ADDONTITLE, "[COLOR %s]%s[/COLOR][COLOR %s] theme zip successful:[/COLOR][COLOR %s]%s[/COLOR]" % (COLOR1, themename, COLOR2, COLOR1, zipname))
    elif type == "addondata":
        if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Are you sure you wish to backup the current addon_data?[/COLOR]" % COLOR2, nolabel="[B][COLOR red]Cancel Backup[/COLOR][/B]", yeslabel="[B][COLOR springgreen]Backup Addon_Data[/COLOR][/B]"):
            ...
            DP.create("[COLOR %s]%s[/COLOR][COLOR %s]: Creating Zip[/COLOR]" % (COLOR1, ADDONTITLE,COLOR2), "[COLOR %s]Creating back up zipPlease Wait...[/COLOR]" % COLOR2)
...
def restoreLocal(type):
    ...
    try:
        ...
        DP.update(0, '[COLOR %s]Unable to read zipfile from current location.[/COLOR]Copying file to packages' % COLOR2)
    ...
    if int(errors) >= 1:
        yes=DIALOG.yesno(ADDONTITLE, '[COLOR %s][COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, fn), 'Completed: [COLOR %s]%s%s[/COLOR] [Errors:[COLOR %s]%s[/COLOR]]Would you like to view the errors?[/COLOR]' % (COLOR1, percent, '%', COLOR1, errors), nolabel='[B][COLOR red]No Thanks[/COLOR][/B]',yeslabel='[B][COLOR springgreen]View Errors[/COLOR][/B]')
        if yes:
            error = six_encode(error)
...
def restoreExternal(type):
    ...
    lookandFeelData('save')
    if int(errors) >= 1:
        yes=DIALOG.yesno(ADDONTITLE, '[COLOR %s][COLOR %s]%s[/COLOR]Completed: [COLOR %s]%s%s[/COLOR] [Errors:[COLOR %s]%s[/COLOR]]Would you like to view the errors?[/COLOR]' % (COLOR2, COLOR1, zname, COLOR1, percent, '%', COLOR1, errors), nolabel='[B][COLOR red]No Thanks[/COLOR][/B]',yeslabel='[B][COLOR springgreen]View Errors[/COLOR][/B]')
        if yes:
            TextBox(ADDONTITLE, error.replace('\t',''))
    setS('installed', 'true')
    setS('extract', str(percent))
...
def whiteList(do):
    ...
    if   do == 'edit':
        ...
        list = whiteList('read')
        if len(list) > 0:
            msg = "Here is a list of your whitelist items, these items(along with dependencies) will not be removed when preforming a fresh start or the userdata overwritten in a build install."
            for item in list:
                try: name, id, fold = item
                except Exception as e: log(str(e))
                msg += "[COLOR %s]%s[/COLOR] [COLOR %s]\"%s\"[/COLOR]" % (COLOR1, name, COLOR2, id)
        ...
        try:
            ...
            DIALOG.ok(ADDONTITLE, "[COLOR %s]Whitelist has been exported to:[/COLOR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, os.path.join(source, 'whitelist.txt')))
        ...
        if not DIALOG.yesno(ADDONTITLE, "[COLOR %s]Are you sure you want to clear your whitelist?This process can't be undone.[/COLOR]" % COLOR2, yeslabel="[B][COLOR springgreen]Yes Remove[/COLOR][/B]", nolabel="[B][COLOR red]No Cancel[/COLOR][/B]"):
        ...
        try:
            for root, dirs, files in os.walk(PACKAGES):
                ...
                if file_count > 0:
                    ...
                    if over: yes=1
                    else: yes=DIALOG.yesno("[COLOR %s]Delete Package Files[/COLOR]" % COLOR2, "[COLOR %s]%s[/COLOR] files found / [COLOR %s]%s[/COLOR] in size.Do you want to delete them?" % (COLOR1, str(file_count), COLOR1, size), nolabel='[B][COLOR red]Don\'t Clear[/COLOR][/B]',yeslabel='[B][COLOR springgreen]Clear Packages[/COLOR][/B]')
                    if yes:
...
def checkSources():
    ...
    if len(match) > 0:
        ...
        for name, path, sharing in match2:
            ...
            DP.update(perc, '[COLOR %s]Checking [COLOR %s]%s[/COLOR]:[/COLOR][COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name, COLOR1, path))
            if 'http' in path:
        ...
        if len(bad) > 0:
            choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]%s[/COLOR][COLOR %s] Source(s) have been found Broken Would you like to Remove all or choose one by one?[/COLOR]" % (COLOR1, len(bad), COLOR2), yeslabel="[B][COLOR springgreen]Remove All[/COLOR][/B]", nolabel="[B][COLOR red]Choose to Delete[/COLOR][/B]")
            if choice == 1:
                remove = bad
            else:
            ...
            if len(remove) > 0:
                ...
                removed = len(remove)
                DIALOG.ok(ADDONTITLE, "[COLOR %s]Checking sources for broken paths has been completed Working: [COLOR %s]%s[/COLOR] | Kept: [COLOR %s]%s[/COLOR] | Removed: [COLOR %s]%s[/COLOR][/COLOR]" % (COLOR2, COLOR2, COLOR1, alive, COLOR1, kept, COLOR1, removed))
...
def killxbmc(over=None):
    if over:
        ...
        choice = DIALOG.yesno('Force Close Kodi', '[COLOR %s]You are about to close KodiWould you like to continue?[/COLOR]' % COLOR2, nolabel='[B][COLOR white] No Cancel[/COLOR][/B]',yeslabel='[B][COLOR white]Force Close Kodi[/COLOR][/B]')
    if choice == 1:
