...
import inspect
from myaccts.modules.control import transPath, setting as getSetting, lang, joinPath, existsPath
...
def log(msg, caller=None, level=LOGINFO):
	...
	try:
		...
		if debug_location == '1':
			log_file = joinPath(LOGPATH, 'myaccts.log')
		...
		traceback.print_exc()
		import xbmc
		xbmc.log('[ script.module.myaccts ] log_utils.log() Logging Failure: %s' % (e), LOGERROR)
...
def error(message=None, exception=True):
	try:
		...
		import xbmc
		xbmc.log('[ script.module.myaccts ] log_utils.error() Logging Failure: %s' % (e), LOGERROR)

def clear_logFile():
	cleared = False
	try:
		from myaccts.modules.control import yesnoDialog
		...
		import xbmc
		xbmc.log('[ script.module.myaccts ] log_utils.clear_logFile() Failure: %s' % (e), LOGERROR)
		cleared = False
	return cleared
...
def view_LogFile(name):
	try:
		...
		from myaccts.modules.control import addonPath
		log_file = joinPath(LOGPATH, '%s.log' % name.lower())
		if not existsPath(log_file):
			from myaccts.modules.control import notification
...
def upload_LogFile():
	...
	try:
		import requests
		from myaccts.modules.control import addonVersion, selectDialog
		f = open(log_file, 'r', encoding='utf-8', errors='ignore')
		text = f.read()
		f.close()
		UserAgent = 'myaccts %s' % addonVersion()
		...
		if 'key' in response.json():
			...
			notification(message='myaccts Log upload failed: %s' % str(response.json()['message']))
			log('My Accounts Log upload failed: %s' % str(response.json()['message']), level=LOGERROR)
		else:
			notification(message='myaccts Log upload failed')
